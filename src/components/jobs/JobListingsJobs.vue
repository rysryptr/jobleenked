...
const searchData = async () => {
  ...
  try {
    ...
    results.value = response.data.filter(job => 
...
<template>
  <section v-if="!state.isLoading" class="bg-white">
    <div class="container mx-auto max-w-7xl py-10 px-6">
      ...
      <div v-if="!state.resultData" class="grid grid-cols-1 md:grid-cols-40/60 w-full gap-6">
        <main>
          <JobsMain 
        ...
        <aside>
          <JobsAside :jobLoading="state.jobLoading" :job="state.job" @deleteJob="fetchData" />
        </aside>
      </div>
      <div v-else-if="state.resultData">
        <div class="min-w-7xl text-center text-gray-500 mx-auto border border-slate-200 p-6 rounded-lg mb-6">
          No data found...
        </div>
      </div>
    </div>
